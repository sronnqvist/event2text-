...
for i in 1 2 3 4 5; do
  ...
  for s in 1 2 3; do
    ...
    python OpenNMT-py/translate.py -seed $seed -gpu 0 -model $BEST -src data/test_manual_$length.input.pcs -output tmp/test_pred_$min_length.$s.txt -replace_unk -max_length $outmax -batch_size $BS -min_length $min_length -length_penalty wu -alpha $ALPHA -verbose -attn_debug > tmp/trans.output
    ...
    grep -n "" tmp/scoring_$min_length.$s.txt |sed -E "s/(^[0-9]+):/\1\t/g" |while read LINE;do echo -e "$length\t$min_length\t$LINE";done > tmp/scoring_$min_length.$s.txt_
...
paste -d "\n" tmp/scoring_*.*.txt|sed "s/ //g" |sed "s/▁/ /g" |python select_generation.py > test_manual_generation.txt
